...
class Part_Object_Scores(object):
    ...
    def generate_layer_type(self):
        auxiliary_layer_sequence = self._net.auxiliary_layer_sequence
        layer_info = self._net.get_layer_info(self._layer_name)
        if self._layer_name in auxiliary_layer_sequence:
            self._layer_type = 'fc'
        elif 'pooling' in layer_info:
        ...
        if self.child_spatial_position is None:
            if self._layer_type == 'fc':
                self.child_spatial_position = \
                    np.array([self.spatial_position])
            else:
                self.child_spatial_position = \
    ...
    def generate_kernel(self, last_filtered_kernel_idx_weight_pairs):
        ...
        if self._layer_type == 'fc':
            ...
            weighted_weight = weight_filtered *\
                np.array(
                    last_filtered_kernel_idx_weight_pairs[1])[..., np.newaxis]
            weighted_weight = weighted_weight[..., np.newaxis, np.newaxis]
        ...
        # if self._layer_type == 'fc':
        # self.kernel[0] = self.kernel[0].transpose((2, 0, 1))

    def get_kernel(self):
        assert self.kernel is not None, 'please generate kernel first'
        return self.kernel

    def generate_children_filtered_kernel_idx_weight_pairs_fc(self,
                                                              negative_slope=0):
        res = []
        res.append()
