...
class Net(object):
    ...
    def get_children_layer_name(self, current_layer_name, include_scale=False):
        if current_layer_name == self.layer_sequence[0]:
            return None
        for idx, layer_name in enumerate(self.layer_sequence):
            if layer_name == current_layer_name:
                return self.layer_sequence[idx - 1]
    ...
    def get_parent_layer_name(self, current_layer_name, include_scale=False):
        if current_layer_name == self.layer_sequence[-1]:
            return None
        for idx, layer_name in enumerate(self.layer_sequence):
            if layer_name == current_layer_name:
                return self.layer_sequence[idx + 1]
    ...
    def generate_layer_info(self, prototxt):
        self._layer_info = parse_net_proto(prototxt)
    ...
    def get_data(self, blob_name, spatial_position, channels_interval, one_img=True, pad=0):
